...
class RateLimiterHttpCallExecutorTest {
    ...
    @Mock private HIBPHttpClient hibpService;
    @Mock private Call<List<Breach>> breachCall;
    @Mock private Call<List<Paste>> pasteCall;
    ...
    @Test
    void getAllPastesForAccount() throws HaveIBeenPwndException, IOException {
        final APIAccount account = APIAccount.builder().key("test-key").plan(APIPlan.RPM10).build();
        final List<RateLimiterEntity> rateLimiter = spy(RateLimiterBuilder.from(account));
        final RateLimiterHttpCallExecutor underTest =
                new RateLimiterHttpCallExecutor(hibpService, rateLimiter);
        List<Paste> expected =
                Response.success(List.of(Paste.builder().title("test-response").build())).body();
        ...
        httpCallExecutorMockedStatic
                .when(() -> HttpCallExecutor.callService(pasteCall))
                .thenReturn(response);
    ...
    @Test
    void getAllBreachesForAccount() throws HaveIBeenPwndException, IOException {
        final APIAccount account = APIAccount.builder().key("test-key").plan(APIPlan.RPM10).build();
        final List<RateLimiterEntity> rateLimiter = spy(RateLimiterBuilder.from(account));
        final RateLimiterHttpCallExecutor underTest =
                new RateLimiterHttpCallExecutor(hibpService, rateLimiter);
        List<Breach> expected =
                Response.success(List.of(Breach.builder().name("test-response").build())).body();
        Optional<List<Breach>> response = Optional.of(expected);

        when(hibpService.getAllBreachesForAccount("test-key", "account", false, false, null))
                .thenReturn(breachCall);
        httpCallExecutorMockedStatic
                .when(() -> HttpCallExecutor.callService(breachCall))
                .thenReturn(response);

        List<Breach> result = underTest.getAllBreachesForAccount("account", null, false, false);

        verify(hibpService, times(1))
                .getAllBreachesForAccount("test-key", "account", false, false, null);
    ...
    @Test
    void getAllBreachesForAccountOverLimit() throws HaveIBeenPwndException, IOException {
        var apiPlan = APIPlan.RPM10;
        ...
        verify(hibpService, times(apiPlan.getRequestsPerMinute()))
                .getAllBreachesForAccount("test-key", "account", false, false, null);
        httpCallExecutorMockedStatic.verify(
                () -> HttpCallExecutor.callService(breachCall),
                times(apiPlan.getRequestsPerMinute()));
    }
}
